...
interface Props {
  ...
  compact?: boolean;
  /** Override the chat container height. Defaults to a tall page layout. */
  heightClass?: string;
}

const AIAssistantChat = ({ compact = false, heightClass }: Props) => {
  ...
  return (
    <div className={compact ? "flex flex-col h-full" : "space-y-6"}>
      {!compact && (
      ...
      <div
        className={`rounded-xl border border-border bg-card flex flex-col ${compact ? "flex-1 min-h-0" : ""}`}
        style={compact ? undefined : { height: heightClass ? undefined : "calc(100vh - 280px)", minHeight: "400px" }}
      >
