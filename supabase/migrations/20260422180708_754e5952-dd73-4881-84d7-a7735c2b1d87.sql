...
-- Update inventory deduction trigger:
-- Stock is still deducted for non-void items.
-- Wastage logging for cancelled/refunded items is now handled client-side
-- (only when staff confirms "food was prepared"), so the trigger no longer
-- auto-creates wastage_logs entries to avoid duplicates.
